...
const PROJECTS = [
  ...
  },
  {
    id: 4,
    title: "AstrCalendar",
    description: "Calendrier cosmique interactif avec phases lunaires et événements astronomiques en temps réel.",
    tags: ["React", "API", "Astronomy"],
    link: "#",
    github: "#",
    featured: false,
    status: "IN_DEV"
        ...
        <div className="text-center lg:text-left lg:grid lg:grid-cols-2 lg:gap-12 items-center">
          <div>
            ...
            <h1 className={`h-32 sm:h-auto text-4xl sm:text-5xl lg:text-6xl font-black tracking-tighter mb-6 uppercase ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>
              {displayText} <span className="animate-blink">_</span> <br />
            ...
            <div className={`flex flex-col sm:flex-row justify-center lg:justify-start gap-4 transition-all duration-1000 delay-700 ${showSubtitle ? 'opacity-100' : 'opacity-0'}`}>
              <a href="#projects" className="group relative inline-flex items-center justify-center px-8 py-3 text-base font-bold text-white transition-all bg-orange-600 hover:bg-orange-700 clip-path-slant">
              ...
              <a href="https://github.com/Greg50100" target="_blank" rel="noopener noreferrer" className={`group inline-flex items-center justify-center px-8 py-3 text-base font-bold border-2 transition-all ${isDarkMode ? 'border-zinc-700 text-zinc-300 hover:border-orange-500 hover:text-orange-500' : 'border-zinc-300 text-zinc-700 hover:border-orange-600 hover:text-orange-600'}`} style={{ clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }}>
...
const ProjectCard = ({ project, isDarkMode, onOpen }) => (
    ...
    <div className="p-6">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {project.tags.map((tag) => (
          <span key={tag} className={`px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider border ${isDarkMode ? 'bg-zinc-950 border-zinc-700 text-zinc-400' : 'bg-zinc-50 border-zinc-300 text-zinc-600'}`}>
            {tag}
          </span>
        ))}
      </div>
...
const Skills = ({ isDarkMode }) => {
  return (
    <motion.section 
      id="skills" 
      className={`py-20 border-y ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {SKILLS.map((skillGroup, index) => (
            <motion.div 
              key={skillGroup.name} 
              className={`p-6 border-l-2 transition-all hover:translate-x-1 ${isDarkMode ? 'bg-zinc-900/50 border-orange-500/50' : 'bg-white border-orange-600/30 shadow-sm'}`}
...
const Contact = ({ isDarkMode }) => {
  ...
  return (
    <motion.section 
      id="contact" 
      className={`py-20 ${isDarkMode ? 'bg-zinc-900' : 'bg-white'}`}
      initial={{ opacity: 0 }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <form onSubmit={handleSubmit} className={`p-1 ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'}`} style={{ clipPath: 'polygon(20px 0, 100% 0, 100% 100%, 0 100%, 0 20px)' }}>
            <div className={`h-full p-8 ${isDarkMode ? 'bg-zinc-950' : 'bg-white'}`}>
                ...
                <input type="text" id="name" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className={`w-full bg-transparent border-b-2 px-2 py-2 outline-none font-mono transition-colors ${isDarkMode ? 'border-zinc-800 text-white focus:border-orange-500' : 'border-zinc-300 text-zinc-900 focus:border-orange-600'}`} placeholder="NOM_UTILISATEUR" />
                </div>
                <div className="mb-6 relative group">
                ...
                <input type="email" id="email" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className={`w-full bg-transparent border-b-2 px-2 py-2 outline-none font-mono transition-colors ${isDarkMode ? 'border-zinc-800 text-white focus:border-orange-500' : 'border-zinc-300 text-zinc-900 focus:border-orange-600'}`} placeholder="EMAIL@DOMAINE.COM" />
                </div>
                ...
                <textarea id="message" rows="4" required value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} className={`w-full bg-transparent border-b-2 px-2 py-2 outline-none font-mono transition-colors ${isDarkMode ? 'border-zinc-800 text-white focus:border-orange-500' : 'border-zinc-300 text-zinc-900 focus:border-orange-600'}`} placeholder="..."></textarea>
                </div>
                <button type="submit" disabled={status === 'sending' || status === 'sent'} className={`w-full py-3 px-6 font-bold uppercase tracking-widest text-white transition-all flex items-center justify-center ${status === 'sent' ? 'bg-green-600' : 'bg-orange-600 hover:bg-orange-700 hover:shadow-[0_0_15px_rgba(234,88,12,0.4)]'}`} style={{ clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }}>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 font-sans ${isDarkMode ? 'bg-zinc-950' : 'bg-zinc-50'}`}>
      ...
      <main>
        ...
        <motion.section 
          id="projects" 
          className={`py-20 border-t ${isDarkMode ? 'border-zinc-900' : 'border-zinc-200'}`}
          initial={{ opacity: 0, y: 40 }}
          whileInView={{ opacity: 1, y: 0 }}
